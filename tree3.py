...
def ensemble_hyperparameter_tuning(X_train, y_train, X_test, y_test):
    ...
    for n_trees, learning_rate, max_depth in product(n_trees_values, learning_rate_values, max_depth_values):
        print(f"正在训练模型: n_trees={n_trees}, learning_rate={learning_rate}, max_depth={max_depth}")
        ensemble = EnsembleRegressionTree(n_trees=n_trees, learning_rate=learning_rate, max_depth=max_depth)
        ensemble.fit(X_train, y_train)  # 训练模型
        y_pred = ensemble.predict(X_test)  # 预测
        mse = mean_squared_error(y_test, y_pred)  # 计算MSE
        print(f"MSE: {mse}")
...
if __name__ == "__main__":
    ...
    ensemble = EnsembleRegressionTree(n_trees=5, learning_rate=0.2, max_depth=10)
    ...
    """
        From the previous results, we observed only a slight improvement in performance 
        when increasing the max_depth from 10 to 15, using n_trees=5 and learning_rate=0.5. 
        Therefore, we have decided to set the max_depth to 10, as it simplifies the tree structure 
        while maintaining comparable performance.
    """
    print("MSE (ensemble with parameters {'n_trees': 5, 'learning_rate': 0.5, 'max_depth': 10}):", 
